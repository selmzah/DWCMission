 ...
 */
  async readEslintConfig(configPath) {
    ...
    switch (configFile) {
      ...
      case "eslint.config.mjs":
        config = this.safeRequire(configPath)
        break;
      default:
        break;
    }
  ...
  async sanitizeEslintConfig(configIn, customRuleExample = false, logger = console) {
    ...
    if (isFilePath) {
      configContents = this.safeRequire(configIn); // TODO error handling invalid json
    }
    ...
    if (isFilePath) {
      await this.writeEslintConfig(configIn, logger);
    }
 ...
 */
   ...
   * @returns {Promise<void>}
   */
  async writeEslintConfig(configPath, logger = console) {
    let configFile = path.basename(configPath);
    switch (configFile) {
      case "eslint.config.js":
      case "eslint.config.cjs":
