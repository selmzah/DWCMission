...
function _initialize(parserContext) {
    ...
    parserContext.vocabNamespaceToAlias = {};
...
function _resolveAnnotationTerm(schemaDataAnnotation, parserContext) {
    ...
    let index = 0;
    while (schemaDataAnnotation.indexOf('@', index) != -1) {

        let i = schemaDataAnnotation.indexOf('@', index);
        let annotationTerm = schemaDataAnnotation.substring(i, schemaDataAnnotation.indexOf('"', i));
...
function _collectBoundedActionFunction(isAction, currNamespace, elementName, elementDataList, parserContext) {
    ...
    for (let i = 0; i < elementDataList.length; i++) {
        ...
        boundedEntityName = entityTypeNamespace + '.' + entityName;
...
function _collectSchemaElements(edmxAsJson, schemaDataList, parserContext) {
    // This array is used to store the parameter values which will be used later to call _collectBoundedActionFunction.
    ...
    // This will delay the execution of _collectBoundedActionFunction, as we need to collect all the entityTypes first.
    boundActionFunctionDetails.forEach(paramValues => {
        _collectBoundedActionFunction(...paramValues, parserContext);
    });
}
